# 调度与状态机

本文档把当前固件里的“轮询”拆开说明：它不是无结构地反复读外设，而是由 1 ms 系统节拍驱动的前台调度状态机，再由各模块维护自己的局部状态机。

## 前台调度器状态机

`ef_scheduler` 对每个任务维护两个核心状态量：

- `elapsed_ms`：距离上次触发已经累计的毫秒数。
- `ready`：任务是否已到期，等待前台执行。

```mermaid
stateDiagram-v2
    [*] --> Waiting: ef_scheduler_init()
    Waiting --> Ready: SysTick累计到 period_ms\nready = true
    Ready --> Running: ef_scheduler_run_once()\n调用 task.run(ctx)
    Running --> Waiting: ready = false\n等待下一轮周期
    Waiting --> Ready: ef_scheduler_mark_ready()
```

中断里只做计数和置位，不直接跑业务；业务都在 `app_run_forever()` 进入的前台循环里执行。

## 调度循环

```mermaid
flowchart TD
    A[main] --> B[ef_platform_init]
    B --> C[ef_log_set_time_fn]
    C --> D[app_start]
    D --> E[ef_scheduler_init]
    E --> F[app_run_forever]
    F --> G[ef_scheduler_run_once]
    G --> H{有 ready 任务?}
    H -- 是 --> I[调用任务 run(ctx)]
    I --> G
    H -- 否 --> J[ef_platform_idle]
    J --> G
```

## App 周期任务

| 任务 | 周期 | 状态机/状态量 | 主要输出 |
| --- | ---: | --- | --- |
| `app_button_task` | 10 ms | `ef_button_t` | 按键事件和状态行 |
| `app_status_page_task` | 100 ms | `g_heartbeat_on`、LCD 脏矩形队列 | 屏幕局部刷新 |
| `app_imu_task` | 5 ms | 零漂状态、低通状态、32 帧 FIFO、姿态快照 | IMU 样本流和 Q15/Q10 姿态输出缓存 |
| `app_encoder_task` | 50 ms | `ef_lowpass_i32_t`、`board_encoder` 计数器 | 两路速度值 |
| `app_speed_control_task` | 50 ms | 目标速度斜坡、误差历史、duty 状态、方向和使能状态 | 左右电机 PWM/DIR 输出 |
| `app_pid_tune_page_task` | 100 ms | 当前选中参数项 | LCD PID 调参页 |
| `app_led_task` | 500 ms | `app_led` 模块任务 | 板载 LED 翻转 |

## 按钮状态机

`ef_button` 是显式状态机，输入是 10 ms 轮询到的原始按下状态，输出是离散事件。

```mermaid
stateDiagram-v2
    [*] --> Released
    Released --> DebouncingDown: raw pressed变化
    DebouncingDown --> Pressed: debounce_ms到期\n输出 DOWN
    Pressed --> LongPressed: 按下超过 long_press_ms\n输出 LONG
    Pressed --> DebouncingUp: raw released变化
    LongPressed --> DebouncingUp: raw released变化
    DebouncingUp --> ClickPending: debounce_ms到期且非长按\n输出 UP
    DebouncingUp --> Released: debounce_ms到期且已长按\n输出 UP
    ClickPending --> Released: click_ms超时\n输出 CLICK
    ClickPending --> DebouncingDown: click_ms内再次按下
    DebouncingDown --> Pressed: debounce_ms到期\n输出 DOWN
    Pressed --> Released: 第二次释放确认\n输出 DOUBLE
```

代码实现没有单独保存 `DebouncingDown/DebouncingUp` 枚举，而是用 `last_raw_pressed`、`stable_pressed` 和 `raw_changed_ms` 表达消抖中间态；`click_pending` 表达等待单击/双击判定的状态。

## 编码器速度状态机

编码器 step 由定时器输入捕获中断累加，App 只在 50 ms 周期读取并清零累计值。速度显示状态由低通滤波器保持。

```mermaid
flowchart TD
    A[Step 捕获中断] --> B[读取 dir GPIO]
    B --> C[board_encoder 累加有符号 count]
    C --> D{50 ms 到期?}
    D -- 否 --> C
    D -- 是 --> E[app_encoder 读取并清零 delta]
    E --> F[ef_lowpass_i32_update]
    F --> G[更新 board_encoder speed_50ms]
    G --> H[刷新 LCD ENC1/ENC2]
```

`ef_lowpass_i32_t` 的状态只有当前输出值 `value`、滤波强度 `shift` 和零点阈值 `zero_threshold`。当前 `shift=1`，即每次采样引入一半新值，50 ms 采样下响应量级约 100 ms。

## 状态页刷新状态机

LCD 状态页使用文本缓存和脏矩形刷新：

1. 各模块调用 `app_status_page_set_line()` 写入对应行的文本缓存。
2. 状态页只重绘被更新的文字区域。
3. `app_status_page_task` 每 100 ms 翻转心跳块，并调用 `board_lcd_service()` 推送脏矩形。

这种方式避免每个业务模块直接控制 LCD 坐标，也避免每次状态变化都阻塞等待整屏刷新。

## 速度环状态机

```mermaid
stateDiagram-v2
    [*] --> Disabled: app_speed_control_init
    Disabled --> Disabled: 50ms tick\nPWM = 0\nPI 状态 reset
    Disabled --> Enabled: PB21 LONG
    Enabled --> Disabled: PB21 LONG
    Enabled --> Enabled: 50ms tick\n读取编码器速度\nQ10 增量式 PI\n输出 PWM/DIR
    Enabled --> Enabled: PB20 LONG\n切换方向并 reset PI 状态
```

PI 参数均为整数保存。`Kp/Ki/Kd` 使用 Q10，`1024` 表示屏幕上的 `1.000`；
目标速度和测量速度单位都是 step/50ms；输出单位是 PWM 千分比。
调参按键定义为：PB20 短按切参数，PB21 短按切步进/小数位，PB05 加，PB04 减，
PB21 长按启停闭环，PB20 长按切换方向。
Target/Dir/Kp/Ki/Kd 参数变化时写入 W25Q128 日志槽；`enabled` 不保存，重启默认关闭。

## IMU 采样状态机

`app_imu` 以 5 ms 前台任务消费 LSM6DSR DMA 采样结果。当前阶段完成 SPI0 DMA burst 读取、实测 `dt`、启动零漂、低通预处理、App 环形 FIFO、Q15 四元数和 Q10 欧拉角输出。

```mermaid
stateDiagram-v2
    [*] --> WaitingDevice: app_imu_init
    WaitingDevice --> WaitingDevice: board_imu_is_ready false
    WaitingDevice --> BiasCollecting: IMU ready
    BiasCollecting --> BiasCollecting: sample_count < 200\n累计 gyro/accel raw
    BiasCollecting --> Running: sample_count >= 200\nbias.complete = true
    Running --> Running: 5ms采样\n实测dt\n扣零漂/低通/FIFO/姿态
```

| 状态/数据 | 保存位置 | 更新条件 |
| --- | --- | --- |
| 设备就绪 | `g_imu_sampling_ready` | `board_imu_is_ready()` 或自动初始化成功 |
| 零漂累计 | `g_gyro_bias_accum[]`、`g_accel_bias_accum[]` | 每帧采样直到 200 帧 |
| 零漂结果 | `g_imu_bias` | 200 帧完成后锁定 |
| 低通状态 | `g_accel_filters[]`、`g_gyro_filters[]` | 每 5 ms 样本更新 |
| FIFO | `g_imu_fifo[]`、`head/tail/count` | 每 5 ms push，消费方 pop |
| 姿态滤波 | `g_attitude_filter` | 零漂完成且 `dt_us != 0` 后更新 Q15/Q10 姿态 |
| 姿态快照 | `g_imu_attitude` | `ef_imu_attitude_update()` 成功后更新，供控制/显示读取 |
