...
| `app_grayscale_task` | 100 ms | 16 位高有效位图、扫描次数、有效标志 | 16 路灰度通道快照 |
| `app_imu_task` | 5 ms | 零漂状态、低通状态、32 帧 FIFO、姿态快照 | `APP_ENABLE_IMU_PIPELINE=1` 时的 IMU 样本流和 Q15/Q10 姿态输出缓存 |
| `app_angle_pid_task` | 10 ms | 单轴 PID、目标角、输出回调 | `APP_ENABLE_IMU_PIPELINE=1` 时的 roll/pitch/yaw 角度环控制量 |
| `app_encoder_task` | 50 ms | `ef_lowpass_i32_t`、`board_encoder` 计数器、编码器快照 | 两路速度值和里程计增量 |
| `app_inav_task` | 50 ms | step Q10 位置、IMU 航向、里程计有效标志 | `APP_ENABLE_IMU_PIPELINE=1` 时的惯导状态快照 |
| `app_motor_task` | 50 ms | 速度环 PID、目标速度、输出回调 | `APP_ENABLE_MOTOR_SPEED_PID=1` 时的电机速度环控制量 |
| `app_led_task` | 500 ms | `ef_event` 事件绑定 | LED 翻转事件 |

## 灰度传感器扫描

灰度仪在前台每 100 ms 扫描一次，扫描时不进入中断。一次扫描保持局部位图，只有 16 路都读完后才更新 App 缓存：

```mermaid
flowchart LR
    A[app_grayscale_task] --> B[通道 0 至 15]
    B --> C[写 S0-S3 地址]
    C --> D[等待 5 us]
    D --> E[读取高有效 AS]
    E --> F{最后一个通道?}
...
地址位序固定为 `S0=bit0`、`S1=bit1`、`S2=bit2`、`S3=bit3`；后续任务只读 `app_grayscale` 缓存，不直接访问 GPIO。
